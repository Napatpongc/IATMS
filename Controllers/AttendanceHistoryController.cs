...
namespace IATMS.Controllers
{
    ...
    public class AttendanceHistoryController : ControllerBase
    {
        private readonly TokenValidationParameters _tokenValidationParameters;
        public AttendanceHistoryController(TokenValidationParameters tokenValidationParameters)
        {
            _tokenValidationParameters = tokenValidationParameters; 
        }
        [HttpGet("getAttHistory")]
        public async Task<IActionResult> getAttHistory([FromQuery] Pay_AttendanceHistory search)
        {
            ...
            try
            {
                if (!AccessRole.IsAuthorize(info.username, menu: "menu_report", function: "func_rp_attendance"))
                {
                    return Forbid();
                }
            }
            ...
            try
            {
                var result = await ConDB.getAttHistory(info.username, search);
                return Ok(result);
